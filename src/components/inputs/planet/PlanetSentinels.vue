...
const groupToFrase: Record<string, string> = {
  Bajo: `[[Sentinel]] activity on this planet is classified as: ''{sentinels}''. The sentinels don't present an immediate threat.`,
  Alto: `[[Sentinel]] activity on this planet is classified as: ''{sentinels}''. The sentinels present an immediate threat.`,
  Agresivo: `[[Sentinel]] activity on this planet is classified as: ''{sentinels}''. The sentinels present an immediate threat.`,
  Corruptos: `[[Sentinel]] activity on this planet is classified as: ''{sentinels}''. The sentinels present an immediate threat.`,
};

watch(model, (newValue) => {
  const group = mappedPlanetSentinels.find((group) => group.items.some((item) => item.value === newValue))?.groupLabel;
  ...
  <InputTableItem>
    <template #label>
      <div class="is-flex is-justify-content-space-between is-align-items-center full-width">
        <label :id>Nivel centinela del planeta:</label>
        <Explainer
          tooltip="Se puede encontrar en el menú de descubrimiento."
          help-img="planet/sentinels"
          help-title="Nivel planetario de los centinelas"
        >
